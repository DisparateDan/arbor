...
function formatGedcomDate(val: unknown): string | null {
  if (!val) return null;
...
function sexToGedcom(val: unknown): string {
...
function gedLine(level: number, tag: string, value = ""): string {
...
function buildFamilies(byName: Record<string, PersonPage>): Family[] {
...
function buildGedcom(byName: Record<string, PersonPage>, filename: string): string {
